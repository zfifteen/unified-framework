...
# Configuration
INPUT_FILES = [f"z_embeddings_{i}.csv" for i in range(1, 11)]
OUTPUT_DIR = "results"
...
def update_stats(chunk, state, sample_frac=0.01):
    ...
    # Initialize or update Welford stats
    if count == 0:
        count = n
        mean = chunk_mean
        M2 = chunk_var * n
    ...
    # Reservoir sampling for quantiles (small random subset)
...
def gather_correlation_samples(chunk, sample_frac, reservoir, max_rows):
    ...
    # If we've exceeded max_rows, trim
...
def main():
    ...
    for fname in INPUT_FILES:
        ...
        for chunk in reader:
            ...
            desc_state = update_stats(chunk, desc_state, sample_frac=0.01)
            # Gather samples for correlation
    ...
    desc_df.to_csv(f"{OUTPUT_DIR}/combined_descriptive_stats.csv")
    ...
    if corr_reservoir:
        ...
        corr_matrix.to_csv(f"{OUTPUT_DIR}/combined_correlation_matrix.csv")
